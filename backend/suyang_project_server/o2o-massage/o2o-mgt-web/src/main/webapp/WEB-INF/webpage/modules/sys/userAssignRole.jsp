...
<script type="text/javascript">
        ...
        $('#uarscontainer').on("dblclick.jstree", ".jstree-anchor", $.proxy(function (e) {
            ...
            if (index == -1 && obj.id != '-1' && !obj.icon) {
                $("#choice").append("<li class=\"select2-selection__choice\">" +
                                    "<div>" + obj.text + "</div>" +
                                    "<span onclick=\"removeChoice(this,'" + obj.id
                                    + "')\" class=\"select2-selection__choice__remove\" tabindex=\"-1\"></span>"
                                    +
                                    "</li>");
                roles.push(obj.id);
            }
    ...
    function removeChoice(choice, id) {
        $(choice).closest(".select2-selection__choice").fadeOut('fast', function () {
            $(this).remove();
            roles = jQuery.grep(roles, function (a) {
                return a != id;
            });
        });
    }
            ...
            <ul class="select2-selection__rendered" id="choice">
                <c:forEach items="${roles}" var="role">
                    <li class="select2-selection__choice">
                        <div>${role.name}</div>
                        <span onclick="removeChoice(this,'${role.id}')"
                           class="select2-selection__choice__remove" tabindex="-1"></span>
                    </li>
                </c:forEach>
            </ul>
...
<div class="modal-footer">
    ...
    <a class="btn btn-primary" id="userARole">保存</a>
</div>