...
var calc_navbar_height = function () {
    ...
    /*
     * App Initialize
     * Description: Initializes the sys with intApp();
     */
    initApp = (function (app) {
            ...
            var smartActions = {
                ...
                toggleMenu: function () {
                    if (!$.root_.hasClass("menu-on-top")) {
                        ...
                        // $.root_.hasClass("mobile-view-activated") ) { suggested fix from
                        // Christian Jäger
                ...
                toggleShortcut: function () {
                    ...
                    $(document).mouseup(function (e) {
                        if (!shortcut_dropdown.is(e.target) && shortcut_dropdown.has(
                                e.target).length === 0) {
                            shortcut_buttons_hide();
                        }
                    });
        ...
        app.domReadyMisc = function () {
            ...
            $('#activity').click(function (e) {
                ...
                var theUrlVal = $this.next('.ajax-dropdown').find('.btn-group > .active > input')
                    .attr('id');
            ...
            $(document).mouseup(function (e) {
                if (!$('.ajax-dropdown').is(e.target) && $('.ajax-dropdown').has(e.target).length
                                                         === 0) {
                ...
                jarvismenu: function (options) {
                    ...
                    $this.find("li a").click(function () {
                        ...
                        if ($(this).parent().find("ul").length !== 0) {
                            ...
                            if ($(this).parent().find("ul:first").is(":visible") && !$(this)
                                    .parent().find("ul:first").hasClass("active")) {
...
function runAllForms() {
    ...
    if ($.fn.autocomplete) {
        $('[data-autocomplete]').each(function () {
            ...
            var $this = $(this),
                availableTags = $this.data('autocomplete') || ["The", "Quick", "Brown", "Fox",
    ...
    if ($.fn.sparkline) {
        ...
        $('.sparkline:not(:has(>canvas))').each(function () {
            ...
            if (sparklineType == 'bar') {
                ...
                sparklineStackedColor =
                    $this.data('sparkline-barstacked-color') || ["#A90329", "#0099c6", "#98AA56",
            ...
            if (sparklineType == 'line') {
                ...
                thisLineColor =
                    $this.data('sparkline-line-color') || $this.css('color') || '#0000f0';
            ...
            if (sparklineType == 'pie') {
                ...
                pieColors =
                    $this.data('sparkline-piecolor') || ["#B4CAD3", "#4490B1", "#98AA56", "#da532c",
                                                         "#6E9461", "#0099c6", "#990099",
                                                         "#717D8A"];
            ...
            if (sparklineType == 'compositeline') {
                ...
                thisMinSpotColor2 =
                    $this.data('sparkline-minspot-color-bottom') || thisMinSpotColor1;
                thisMaxSpotColor2 =
                    $this.data('sparkline-maxspot-color-bottom') || thisMaxSpotColor1;
                thishighlightSpotColor1 =
                    $this.data('sparkline-highlightspot-color-top') || '#50f050';
                thisHighlightLineColor1 =
                    $this.data('sparkline-highlightline-color-top') || '#f02020';
...
function setup_widgets_desktop() {
    ...
    if ($.fn.jarvisWidgets && enableJarvisWidgets) {
        ...
        $('#widget-grid').jarvisWidgets({
                                            ...
                                            ajaxnav: $.navAsAjax // declears how the localstorage
                                                                 // should be saved (HTML or AJAX
                                                                 // Version)
...
function loadScript(scriptName, callback) {
    if (!jsArray[scriptName]) {
        ...
        root.root.console.log("This script was already loaded %c: " + scriptName,
            debugStyle_warning);
    }
...
function checkURL() {
    ...
    if (!url) {
        ...
        try {
            ...
            if (documentUrl) {
                if (documentUrl.indexOf('#', 0) > 0 && documentUrl.indexOf('#', 0)
...
function loadURL(url, container) {
    ...
    $.ajax({
               ...
               url: url,
               dataType: 'html',
               cache: true, // (warning: setting it to false will cause a timestamp and will call
                            // the request twice)
               beforeSend: function () {

                   //IE11 bug fix for googlemaps (delete all google map instances)
                   //check if the page is ajax = true, has google map class and the container is
                   // #content
                   ...
                   if ($.navAsAjax && (container[0] == $("#content")[0]) && enableJarvisWidgets
                       && $("#widget-grid")[0]) {
                   ...
                   if ($.navAsAjax && (container[0] == $("#content")[0])) {
                       ...
                       // destroy form controls: Datepicker, select2, autocomplete, mask, bootstrap
                       // slider
                   ...
                   if (container[0] == $("#content")[0]) {
                       ...
                       $('body').find('> *').filter(':not(' + ignore_key_elms + ')').empty()
                           .remove();
               ...
               error: function (xhr, status, thrownError, error) {
                   container.html('<h4 class="ajax-loading-error"><i class="fa fa-warning txt-color-orangeDark"></i> Error requesting <span class="txt-color-red">'
                                  + url + '</span>: ' + xhr.status
                                  + ' <span style="text-transform: capitalize;">' + thrownError
                                  + '</span></h4>');
               },
               async: true
           });
...
function drawBreadCrumb(opt_breadCrumbs) {
    ...
    bread_crumb.empty(),
        ...
        bread_crumb.append($("<li></li>").html(
            $.trim($(this).clone().children(".badge").remove().end().text()))), --b
                                                                                || (document.title =
            bread_crumb.find("li:last-child").text())
    });
...
function ajaxSubmit(form, dataType, callback) {
    var $form = $(form);
    $.ajax({
...
function ajaxError(xhr) {
    message_box.show(xhr.responseText, "error");
}

function loadContent(node, url, dataType) {
    $.ajax({
       type: "get",
       async: false,
       dataType: dataType || "html",
...
function validateForm(form, submitCallback) {
